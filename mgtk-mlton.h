#ifndef __MGTK_ML_H__
#define __MGTK_ML_H__
...
#if !defined(PART_OF_MGTK) && \
    !defined(STATIC_LINK_MGTK) && \
    !defined(DYNAMIC_LINK_MGTK)
#define PART_OF_MGTK
#endif
...
#elif defined(STATIC_LINK_MGTK)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(DYNAMIC_LINK_MGTK)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) EXTERNAL x
#else
#error Must specify linkage for mgtk
#define MLLIB_PRIVATE(x)
...
MLLIB_PUBLIC(void mgtk_callback_dispatch_smlside (Int32 x0, CPointer x1, CPointer x2, Int32 x3);)
MLLIB_PUBLIC(void mgtk_callback_destroy_smlside (Int32 x0);)
